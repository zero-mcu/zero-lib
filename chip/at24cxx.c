...
static int at24cxx_write(at24cxx_t* at24cxx, ze_u32_t address, const ze_u8_t* wr_data, ze_size_t len)
{
    ASSERT(at24cxx);
    ASSERT(address >= 0);
    ASSERT(wr_data);
    ...
    while(length > 0)
    {
        ze_u8_t dev_addr = at24cxx->dev_addr | ((address >> 8) & chip_info->dev_mask);
        ...
        wr_data += wr_len;
        address += wr_len;
        sys_sleep_ms(chip_info->mdelay);
    }

    return (len - length);
}

static int at24cxx_read(at24cxx_t* at24cxx, ze_u32_t address, ze_u8_t* rd_data, ze_size_t len)
{
    ASSERT(at24cxx);
    ASSERT(address >= 0);
    ASSERT(rd_data);
...
static int at24cxx_eeprom_write(eeprom_t* eeprom, ze_u32_t address, const ze_u8_t* wr_data, ze_size_t wr_len)
{
    ASSERT(eeprom);
    ASSERT(wr_data);
...
static int at24cxx_eeprom_read(eeprom_t* eeprom, ze_u32_t address, ze_u8_t* rd_data, ze_size_t rd_len)
{
    ASSERT(eeprom);
    ASSERT(rd_data);
...
void at24cxx_init(eeprom_t* eeprom, at24cxx_t* at24cxx, i2c_t* i2c, ze_u16_t dev_addr, at24cxx_chip_t chip_type)
{
    ASSERT(eeprom);
    ASSERT(at24cxx);
    ASSERT(i2c);
